...
void loadData(const char* filename, sdds::Autoshop& as)
{
	...
	while (file)
	{
		...
		try { 
			...
			std::cout << "Unrecognized record type : [";
			std::cout << c;
			std::cout << "]" << std::endl;
		}
		if (aVehicle)
...
int main(int argc, char** argv)
{
	...
	std::list<const sdds::Vehicle*> vehicles;
	{
		...
		auto fastVehicles = [](const sdds::Vehicle* vp) { return vp->topSpeed() > 300 ? true : false; };
	...
	std::cout << std::endl;
	{
		...
		auto brokenVehicles = [](const sdds::Vehicle* vp) { return vp->condition() == "broken" ? true : false; };
